import React from 'react';
import { store } from '../services/store';

interface SidebarProps {
  currentTab: string;
  setCurrentTab: (tab: string) => void;
}

export const Sidebar: React.FC<SidebarProps> = ({ currentTab, setCurrentTab }) => {
  const currentUser = store.users.find(u => u.id === store.currentUserId) || store.users[0];
  const currentMember = store.members.find(
    m => m.tenantId === store.activeTenantId && m.userId === currentUser.id
  ) || store.members[0];

  const roleLabels: Record<string, string> = {
    ORGANIZER: 'Admin Team',
    COACH: 'Coach',
    PLAYER: 'Pro Player'
  };

  const navItems = [
    { id: 'painel', label: 'Dashboard', icon: 'dashboard' },
    { id: 'partidas', label: 'Matches', labelBr: 'Partidas', icon: 'sports_esports' },
    { id: 'mapas', label: 'Maps', labelBr: 'Mapas', icon: 'map' },
    { id: 'jogadores', label: 'Players', labelBr: 'Jogadores', icon: 'groups' },
    { id: 'treinamento', label: 'Training', labelBr: 'Treinamento', icon: 'fitness_center' },
    { id: 'conteudos', label: 'Content', labelBr: 'Conteúdos', icon: 'movie' },
    { id: 'analises', label: 'Analysis', labelBr: 'Análises', icon: 'analytics' },
    { id: 'equipe', label: 'Team', labelBr: 'Equipe', icon: 'badge' },
    { id: 'configuracoes', label: 'Settings', labelBr: 'Configurações', icon: 'settings' },
  ];

  return (
    <aside className="w-[260px] h-full fixed left-0 top-0 bg-[#ffffff] border-r border-[#ccc3d8] flex flex-col py-6 gap-2 z-50">
      
      {/* Brand Header */}
      <div className="px-6 mb-6 flex items-center gap-3">
        <div className="w-10 h-10 flex items-center justify-center bg-[#630ed4] rounded-lg text-white font-extrabold text-xl shadow-xs">
          V
        </div>
        <div className="flex flex-col">
          <h1 className="font-bold text-lg text-[#630ed4] leading-tight">VALORANT OPS</h1>
          <p className="text-[10px] uppercase tracking-widest text-[#7b7487] font-bold">Elite Performance</p>
        </div>
      </div>

      {/* Navigation List */}
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => {
          const isActive = currentTab === item.id || (currentTab === 'jogadores-perfil' && item.id === 'jogadores');

          return (
            <button
              key={item.id}
              onClick={() => setCurrentTab(item.id)}
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-all rounded-lg text-left ${
                isActive
                  ? 'bg-[#e3e1ed] text-[#630ed4] font-semibold'
                  : 'text-[#4a4455] hover:bg-[#f9f1ff] hover:text-[#630ed4]'
              }`}
            >
              <span className={`material-symbols-outlined text-[20px] ${isActive ? 'fill-1' : ''}`}>
                {item.icon}
              </span>
              <span>{item.labelBr || item.label}</span>
            </button>
          );
        })}
      </nav>

      {/* User Profile Widget at Bottom */}
      <div className="px-5 pt-4 border-t border-[#ccc3d8]">
        <div className="flex items-center gap-3 p-2.5 rounded-lg bg-[#f3ebfa] border border-[#e8dfee]">
          <div className="w-8 h-8 rounded-full bg-[#eaddff] flex items-center justify-center text-[#630ed4] font-bold text-xs">
            {currentUser.fullName.substring(0, 1)}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold text-[#1d1a24] truncate">{currentUser.fullName}</p>
            <p className="text-[10px] text-[#4a4455] truncate">{roleLabels[currentMember?.role || 'PLAYER']}</p>
          </div>
          <span className="material-symbols-outlined text-[#7b7487] text-sm cursor-pointer hover:text-[#630ed4]">
            logout
          </span>
        </div>
      </div>

    </aside>
  );
};
